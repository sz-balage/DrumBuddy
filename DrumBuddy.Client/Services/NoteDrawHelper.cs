...
public class NoteDrawHelper
{
    ...
    private double FromCentreBasedToTopLeftCoordinateX(double coordinateX) => coordinateX + _canvasWidth / 2;
    private double FromCentreBasedToTopLeftCoordinateY(double coordinateY) => coordinateY + _canvasHeight / 2;
    private static bool AreOneDrumAway(Drum drum, Drum otherDrum) => Math.Abs(GetYPositionForDrum(drum) - GetYPositionForDrum(otherDrum)) == (NoteHeadSize.Height / 2);
    ...
    private LineAndStroke GetLineForNoteGroup(NoteGroup noteGroup, double xPosition)
    {
        ...
        return new LineAndStroke(new Point(FromCentreBasedToTopLeftCoordinateX(xPosition) + 12, startPointY),new Point(FromCentreBasedToTopLeftCoordinateX(xPosition ) +12, endPointY), LineThickness);
    }
    /// <summary>
    ...
    private static (Uri Path, Size ImageSize) GetCircleImagePathAndSize() => (new Uri(BaseNotationPath + "circle" + ".png"), CircleSize);
    ...
    internal (ImmutableArray<NoteImageAndBounds> Images,ImmutableArray<LineAndStroke> LineAndStrokes) GenerateLinesAndNoteImages(
        Func<Note, Point, NoteImageAndBounds> getNoteImage,
        Func<Point, NoteImageAndBounds> getCircleImage,
        RythmicGroup rythmicGroup,
        double noteGroupWidth,
        double startingXPosition)
    {
        var linesAndStrokes = new List<LineAndStroke>(); //TODO: draw lines
        ...
        GetLinesAndImagesToDraw(RythmicGroup rythmicGroup)
    {
        ...
        var noteGroupWidth = _canvasHeight / 4;
        ...
        return GenerateLinesAndNoteImages(getNoteImage, getCircleImage, rythmicGroup, noteGroupWidth, startingXPos);
    }
